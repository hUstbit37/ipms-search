...
export default function TransferCreatePage() {
  ...
  const goToStep = (step: number) => {
    ...
    try {
      if (typeof window === "undefined") {
        router.push("/contracts/transfers/create");
        return;
      }
      ...
      router.push(
        `/contracts/transfers/create${query ? `?${query}` : ""}`,
        { scroll: false },
      );
    } catch (error) {
  ...
  useEffect(() => {
    try {
      ...
      const stored = localStorage.getItem("transfer_general_info");
      if (!stored) return;
      const parsed = JSON.parse(stored) as Partial<GeneralInfoForm>;
      setValue("transfer_method", parsed.transfer_method || "");
      setValue("transfer_type", parsed.transfer_type || "");
          ...
          <p className="text-sm text-muted-foreground">
            {currentStep === 1
              ...
              : currentStep === 2
                ...
                : currentStep === 3
                  ? "Cấu hình các điều khoản và phí chuyển nhượng"
                  : currentStep === 4
