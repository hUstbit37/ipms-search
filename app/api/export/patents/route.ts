 ...
 */
const formatAgency = (item: PatentItem): string => {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const baseColumns = [
      ...
      { header: 'Chủ đơn', key: 'owner', width: 50, style: { alignment: { vertical: 'middle' as const, wrapText: true } } },
      { header: 'Phân loại IPC', key: 'ipc_list', width: 30, style: { alignment: { vertical: 'middle' as const, wrapText: true } } },
    ...
    for (let index = 0; index < items.length; index++) {
      ...
      const rowData: Record<string, any> = {
        ...
        ipc_list: (item.ipc_list_raw?.join(", ")) || (Array.isArray(item.ipc_list) ? item.ipc_list.join(", ") : item.ipc_list || '-'),
        status:
          item.wipo_status ||
