...
import { useAllAgencies } from "@/hooks/useAgencyQuery";
import {
  IP_TYPES,
  IP_TYPE_LABELS,
  type IpType,
...
const partnersSchema = z.object({
  licensor_organization_id: z
    .string()
    .min(1, "Vui lòng chọn bên chuyển nhượng"),
  licensee_organization_id: z
    .string()
    .min(1, "Vui lòng chọn bên nhận chuyển nhượng"),
...
// Form chọn các bên tham gia và danh mục IP áp dụng cho hợp đồng chuyển nhượng
export default function PartnersStepForm({ onBack, onNext }: PartnersStepFormProps) {
  ...
  const { agencies, isLoading: isLoadingAgencies } = useAllAgencies();
  ...
  useEffect(() => {
    try {
      ...
      const stored = localStorage.getItem("transfer_partners_info");
      if (!stored) return;
  ...
  const persistDraft = (values: PartnersStepFormValues) => {
    try {
      if (typeof window === "undefined") return;
      localStorage.setItem("transfer_partners_info", JSON.stringify(values));
    } catch (error) {
      ...
      <div className="grid md:grid-cols-1 gap-4">
        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-700">
            Bên chuyển nhượng <span className="text-red-500">*</span>
          </label>
          <Controller
            control={control}
            ...
            render={() => (
              <BaseSelect
                ...
                placeholder="Chọn bên chuyển nhượng"
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-700">
            Bên nhận chuyển nhượng <span className="text-red-500">*</span>
          </label>
          <Controller
            control={control}
            ...
            render={() => (
              <BaseSelect
                ...
                placeholder="Chọn bên nhận chuyển nhượng"
